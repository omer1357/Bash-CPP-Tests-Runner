...
if [ ! -d "./Tests" ]
	then
		printf "Tests folder not found"
		exit
	fi

dos2unix ./Tests/* >/dev/null 2>&1
...
g++ -o compiled.exe -g -std=c++11 -DNDEBUG -Wall *.cpp
...
do
	...
	valgrind --log-file=${i%.*}.valgrind_log --leak-check=full ./compiled.exe < $i > ${i%.*}.vresult 1>/dev/null 2>/dev/null
	rm ${i%.*}.vresult
	if [ -f ${i%.*}.valgrind_log ]
	then
